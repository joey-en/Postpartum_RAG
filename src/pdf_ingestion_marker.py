...
r'''
python src\pdf_ingestion_marker.py OR src\pdf_ingestion_marker.py SRC001.pdf

scans data\raw_pdfs\*.pdf then writes parsed markdown to
data\parsed_pdf_marker\{same_name}.md. skips any PDF that already
has a matching `.md`
'''


ROOT_DIR = Path(__file__).resolve().parents[1]
PARSED_PDF_MARKER_DIR = ROOT_DIR / "data" / "parsed_pdf_marker"
CHUNK_PAGE_COUNT = 30
...
def load_pdf_parser():
    try:
        ...
        message = (
            ...
            "Install it first with `pip install marker-pdf`."
        )
...
def parse_pdf_to_markdown(pdf_path: Path, to_markdown) -> str:
    ...
    with tempfile.TemporaryDirectory(prefix="marker_pdf_chunks_") as temp_dir_name:
...
def main() -> None:
    ...
    processed_count, pending_skipped_count, failed_count = process_pdfs_one_by_one(
        ...
        parsed_dir=PARSED_PDF_MARKER_DIR,
